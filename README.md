...
1. 用微信开发者工具导入 `apps/miniprogram`（项目名：牛马上钟）。
2. AppID 使用 `wxdd407d232b60aff1` 或替换为自己的 AppID。
...
- 微信登录未填写昵称时，会自动获取微信昵称与头像（受微信隐私策略影响，可能返回匿名昵称与默认头像）；昵称输入框也支持微信昵称快捷填充。
...
- 下班提醒：提醒设置页可分别配置上班/下班提前提醒分钟数（多选），与上班提醒共用同一个订阅模板。
  一次性订阅消息发送后会失效，需再次订阅；当前仓库配置为 `SUBSCRIBE_MINIPROGRAM_STATE=formal`（正式版），
  本地/开发调试时可临时改为 `developer`。
...
- 未配置订阅消息模板 ID 时，提醒页显示“暂未配置模板”，云端也不会生成提醒任务；配置后 `dispatcher`
  的定时触发器（每 5 分钟）自动扫描并发送微信订阅消息。天气使用 Open-Meteo 真实接口，不依赖 mock。
